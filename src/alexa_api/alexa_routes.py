...
This module registers HTTP endpoints on a provided Flask Blueprint.
It reads from the shared store so that URLs pushed by Music Assistant
are available to Alexa.
"""
...
def register_routes(bp):
    ...
    def latest_url():
        ...
        if not shared_store._store:
            return jsonify({
                'error': 'Check skill invocations and skill logs. '
                         'If there are no invocations, you have made a configuration error'
            }), 404
